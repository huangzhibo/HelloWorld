...
class haplotypeCaller(Workflow):
    ...
    #    bed_dir = impl.mkdir(self.option.workdir, "temp", "bed")
        ...
        mapper = ["#!/bin/sh",
                  ...
                  "\t\tsleep 3",
                  '\t\tjava -Xmx%sm -jar %s -T GenotypeGVCFs --variant ${VCF_TMP}/$base.g.vcf.gz -R %s -o ${VCF_TMP}/$base.hc.vcf.gz' %
                  (self.haplotypeCaller.mapper_mem, self.haplotypeCaller.program, self.ref.normal.ref),
                  "\telse",
                  '\t\techo "Empty LANE"',
                  "\tfi",
                  "done"
