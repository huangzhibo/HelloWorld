            ...
            if mode != 3 and mode != 4:
                if self.option.multiSample:
                    ...
                    for sampleName in sampleInfo.keys():
                        ...
                        cmd.append("check.py -s %s/state.json -n %s %s" % (self.stateDir, sampleName, self.init.check_state_param))
                        for dataTag in sample.keys():
                            laneData = os.path.join(self.option.dirHDFS, 'fq', sampleName, dataTag)
