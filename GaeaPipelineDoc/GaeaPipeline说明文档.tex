...
\path (-5.5,0.5) node(sample) [io] {sampleList}
(-5.5,-1.2) node(config) [io] {user.config};
...
\label{sec:5}
\subsection{简述}
GaeaPipeline的一个APP模块就是一个按约定书写的python脚本，将写好的APP放到{\kaishu \hyperref[envset]{\$GAEA\_HOME}}/workflow目录下或在user.conf中指定存放APP的路径即可使用。使用时只需在user.conf中设置``analysis\_flow''即可{\kaishu \hyperref[fig:flow]{(如图)}}。
...
\lstinputlisting[language=bash]{source/example.json}
\end{itemize}
...
流程的分析步骤需要在user.config中设置，参数名为``analysis\_flow"。{\kaishu \hyperref[fig:flow]{下图}}是user.conf中``analysis\_flow"参数的设置形式，其中只有第一行的起始步骤没有依赖。另外，对执行步骤只依赖于前一步骤，且属于同一平台的步骤列表可以合并, 步骤间用``|''分割，代表前一步骤的输出是后一步骤的输入。

\begin{figure}[H]
 \label{fig:flow}
%\begin{tikzpicture}[x=2 cm,y=1 cm]
\begin{tikzpicture}
\tikzstyle{markbox}=[draw=red,text width=1em, minimum height=1em] 
%\path (0,0) node(usage)[blankbox,fill=gray!0,text width=14.5cm]{\lstinputlisting{source/workflow.json}};
\path (-3.9,0) node (left-bound) []{\lstinputlisting{source/workflow.json}};
\path (-3.9,-4.5) node (left-bound) []{\lstinputlisting{source/simple_workflow.json}};
\path (-3.9,-8.8) node (left-bound) []{\lstinputlisting{source/workflow.cfg}}
(-1,-7.2) node(ann) [text width=25em,minimum height=2em] {cfg格式，stageN仅作为key标记，没有实际意义} ;
\path (-4.2,0.2) node(boxmark1) [markbox] {}
(-3.45,0.2) node(boxmark2) [markbox] {}
(-2.7,0.2) node(boxmark3) [markbox] {}
(2.2,1.9) node(ann) [text width=17em,minimum height=2em] {脚本运行平台} 
(2.2,1) node(ann2) [text width=17em,minimum height=2em] {运行的步骤（APP）}
(2.2,-1) node(ann3) [text width=17em,minimum height=3em] {运行步骤所依赖的步骤（APP），依赖步骤的输出将做为运行步骤的输入，单机步骤(S)将依此设置提交任务的依赖关系。};
\draw[very thick,blue,->] (boxmark1)..controls (-3,1.6) and (-1,1.9) ..(ann);
\draw[very thick,blue,->] (boxmark2)..controls (-2,1.2) and (-1,1)..(ann2);
\draw[very thick,blue,->] (boxmark3)..controls (-1.7,0.4) and (-1.3,-0.4) ..(ann3);
...
%\section{目录结构}
%\subsection{流程目录结构}
%\lstinputlisting[language=TeX]{source/pipelineDirTree.txt}
...
\label{sec:3}
\subsubsection{Usage}
Gaea.py是Gaea流程主程序，将调用其他模块，并根据输入信息生成脚本和其他信息。RunGaea是用shell对Gaea.py进行的包装，可以直接运行，不用配置环境变量\footnote{若使用Gaea.py需要设置环境变量，具体设置见GaeaPipeline/config/envset.sh}。
\lstinputlisting[language=bash,keywordstyle=\color{black}]{gaea_usage.txt}
